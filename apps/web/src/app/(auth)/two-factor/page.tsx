      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <div className="rounded-2xl bg-white p-8 shadow-soft-sm md:p-10">
          ...
          <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-cream-100">
            <Lock className="h-8 w-8 text-terracotta-600" />
          </div>

          <h1 className="text-center font-serif text-3xl font-bold text-charcoal-900">Two-Factor Authentication</h1>
          <p className="mt-2 text-center text-sm text-warm-gray-500">Enter the 6-digit code from your authenticator app.</p>
          ...
          <div className="mt-6 flex justify-center gap-2">
            {code.map((digit, i) => (
              <input
                ...
                className={cn("h-14 w-12 rounded-lg border text-center font-serif text-xl font-bold text-charcoal-900 focus:border-terracotta-500 focus:ring-2 focus:ring-terracotta-500/20 focus:outline-none", digit ? "border-terracotta-400" : "border-charcoal-200")}
              />
            ))}
          </div>
          ...
          <button
            ...
            className="mt-6 flex h-12 w-full items-center justify-center rounded-lg bg-terracotta-600 text-sm font-semibold text-white shadow-soft transition-all hover:bg-terracotta-700 disabled:cursor-not-allowed disabled:opacity-60"
          >
          ...
          <div className="mt-6 text-center space-y-2">
            ...
            <p><Link href="/sign-in" className="inline-flex items-center gap-1 text-sm text-warm-gray-500 hover:text-charcoal-600"><ArrowLeft className="h-3 w-3" /> Back to Sign In</Link></p>
          </div>
        </div>
      </motion.div>
    </div>
