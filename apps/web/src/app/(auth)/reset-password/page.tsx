...
export default function ResetPasswordPage() {
  ...
  if (!token) {
    return (
      <div className="relative flex min-h-screen items-center justify-center bg-gradient-to-br from-surface-50 via-white to-accent-50 px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
          <div className="rounded-3xl bg-white/80 backdrop-blur-xl p-8 md:p-10 shadow-[0_20px_60px_-20px_rgba(176,86,110,0.3)] border border-white/50 text-center">
            <h1 className="font-serif text-2xl font-bold text-text-900">Invalid Reset Link</h1>
            <p className="mt-2 text-sm text-muted-500">This password reset link is missing or invalid. Please request a new one.</p>
            <Link href="/forgot-password" className="mt-5 inline-flex items-center justify-center h-11 px-6 rounded-xl bg-primary-500 text-sm font-medium text-white shadow-sm hover:bg-primary-600 transition-colors">
              Request New Link
            </Link>
          </div>
        </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="relative w-full max-w-md">
        <div className="rounded-3xl bg-white/80 backdrop-blur-xl p-8 md:p-10 shadow-[0_20px_60px_-20px_rgba(176,86,110,0.3)] border border-white/50">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div>
              <label htmlFor="password" className="block text-sm font-medium text-text-700 mb-1.5">New Password</label>
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPw(!showPw)} className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-400 hover:text-text-600 transition-colors">
            ...
            <div>
              <label htmlFor="confirmPassword" className="block text-sm font-medium text-text-700 mb-1.5">Confirm Password</label>
              <div className="relative">
                <Lock className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-400" />
                ...
                <button type="button" onClick={() => setShowCpw(!showCpw)} className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-400 hover:text-text-600 transition-colors">
