          ...
          {status === "success" ? (
            <div className="text-center">
              ...
              <button
                ...
                disabled={cooldown > 0}
                className={cn("mt-4 text-sm font-medium", cooldown > 0 ? "text-warm-gray-400" : "text-terracotta-600 hover:text-terracotta-700")}
              >
            ...
            <>
              ...
              <p className="mt-2 text-center text-sm text-warm-gray-500">Enter your email and we&apos;ll send you a reset link.</p>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input id="email" type="email" autoComplete="email" {...register("email")} placeholder="you@example.com" className={cn("w-full rounded-lg border py-3 pl-10 pr-4 text-sm focus:border-terracotta-500 focus:ring-2 focus:ring-terracotta-500/20 focus:outline-none", errors.email ? "border-red-400" : "border-charcoal-200")} />
                  </div>
                  {errors.email && <p className="mt-1 text-xs text-red-500">{errors.email.message}</p>}
                </div>
                <button type="submit" disabled={isPending} className="flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-terracotta-600 text-sm font-semibold text-white shadow-soft transition-all hover:bg-terracotta-700 disabled:cursor-not-allowed disabled:opacity-60">
              ...
              <p className="mt-6 text-center text-sm text-warm-gray-500">
                <Link href="/sign-in" className="font-medium text-terracotta-600 hover:text-terracotta-700">Back to Sign In</Link>
              </p>
            </>
          )}
        </div>
