  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-charcoal-900">Competitions</h1>
        <button
          type="button"
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-cream-100">
              {competitions.map((comp) => (
                <tr key={comp.id}>
                  <td className="px-4 py-3 font-medium text-charcoal-900">{comp.title}</td>
                  <td className="px-4 py-3">
                    <Badge variant={comp.status === "ACTIVE" ? "teal" : comp.status === "UPCOMING" ? "amber" : "gray"} size="sm">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      <button type="button" onClick={() => { setEditing(comp); setShowModal(true); }} className="p-1 text-warm-gray-500 hover:text-terracotta-600"><Pencil className="w-3.5 h-3.5" /></button>
                      <button type="button" onClick={() => deleteMutation.mutate(comp.id)} className="p-1 text-warm-gray-500 hover:text-red-600"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <AnimatePresence>
        {showModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-charcoal-900/50">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative bg-white rounded-xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto p-6">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
                ...
                <div>
                  ...
                  <textarea {...register("description")} rows={3} className="w-full px-3 py-2 border border-cream-200 rounded-lg text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-charcoal-700 mb-1">Craft Type</label>
                  <select {...register("craftType")} className="w-full px-3 py-2 border border-cream-200 rounded-lg text-sm">
                    <option value="">All</option>
                    {CRAFT_TYPES.map((ct) => <option key={ct.value} value={ct.value}>{ct.label}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-charcoal-700 mb-1">Prize Description</label>
                  <input {...register("prizeDescription")} className="w-full px-3 py-2 border border-cream-200 rounded-lg text-sm" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-sm font-medium text-charcoal-700 mb-1">Start Date</label>
                    <input type="date" {...register("startDate")} className="w-full px-3 py-2 border border-cream-200 rounded-lg text-sm" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-charcoal-700 mb-1">End Date</label>
                    <input type="date" {...register("endDate")} className="w-full px-3 py-2 border border-cream-200 rounded-lg text-sm" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-charcoal-700 mb-1">Max Entries</label>
                  <input type="number" {...register("maxEntries", { valueAsNumber: true })} className="w-full px-3 py-2 border border-cream-200 rounded-lg text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-charcoal-700 mb-1">Status</label>
                  <select {...register("status")} className="w-full px-3 py-2 border border-cream-200 rounded-lg text-sm">
