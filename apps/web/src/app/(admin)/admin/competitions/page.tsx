                ...
                <tr key={comp.id}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      <button type="button" onClick={() => { setEditing(comp); setShowModal(true); }} className="p-1 text-muted-500 hover:text-rose-600"><Pencil className="w-3.5 h-3.5" /></button>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative bg-white rounded-xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto p-6">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
                ...
                <div>
                  ...
                  <textarea {...register("description")} rows={3} className="w-full px-3 py-2 border border-blush-200 rounded-lg text-sm" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input {...register("prizeDescription")} className="w-full px-3 py-2 border border-blush-200 rounded-lg text-sm" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-sm font-medium text-charcoal-700 mb-1">Start Date</label>
                    <input type="date" {...register("startDate")} className="w-full px-3 py-2 border border-blush-200 rounded-lg text-sm" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-charcoal-700 mb-1">End Date</label>
                    <input type="date" {...register("endDate")} className="w-full px-3 py-2 border border-blush-200 rounded-lg text-sm" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-charcoal-700 mb-1">Max Entries</label>
                  <input type="number" {...register("maxEntries", { valueAsNumber: true })} className="w-full px-3 py-2 border border-blush-200 rounded-lg text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-charcoal-700 mb-1">Status</label>
                  <select {...register("status")} className="w-full px-3 py-2 border border-blush-200 rounded-lg text-sm">
