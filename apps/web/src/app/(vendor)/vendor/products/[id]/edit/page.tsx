  ...
  if (isError || !product) {
    return (
      <div className="text-center py-16">
        <p className="text-lg text-warm-gray-600">Product not found</p>
        <Link href="/vendor/products" className="text-terracotta-600 font-medium mt-2 inline-block">
          Back to Products
        </Link>
      </div>
    );
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-charcoal-900">Edit Product</h1>
        <Badge
          variant={
            product.status === "PUBLISHED" ? "teal" : product.status === "DRAFT" ? "gray" : "amber"
          }
        >
      ...
      <form id="edit-product-form" onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="bg-white rounded-xl border border-cream-200 p-6 space-y-4">
          ...
          <div>
            ...
            <input {...register("title")} className="w-full px-3 py-2 border border-cream-200 rounded-lg text-sm focus:ring-2 focus:ring-terracotta-500" />
          ...
          <div>
            ...
            <textarea {...register("shortDescription")} rows={2} className="w-full px-3 py-2 border border-cream-200 rounded-lg text-sm" />
          </div>
        ...
        <div className="bg-white rounded-xl border border-cream-200 p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-charcoal-700 mb-1">Product Images</label>
            <FileUpload maxFiles={10} accept="image/*" />
            {product.images && product.images.length > 0 && (
              <div className="grid grid-cols-4 gap-2 mt-3">
                {product.images.map((img) => (
                  <div key={img.id} className="relative rounded-md overflow-hidden border border-cream-200">
                    <img src={img.thumbnail || img.url} alt={img.alt || product.title} className="w-full aspect-square object-cover" />
                    {img.isPrimary && (
                      <span className="absolute top-1 left-1 bg-terracotta-600 text-white text-[10px] px-1.5 py-0.5 rounded">
                        Primary
                      </span>
                    )}
                  </div>
        ...
        <div className="bg-white rounded-xl border border-cream-200 p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" step="0.01" {...register("price")} className="w-full px-3 py-2 border border-cream-200 rounded-lg text-sm" />
              {errors.price && <p className="text-xs text-red-600 mt-1">{errors.price.message}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Compare-at Price</label>
              <input type="number" step="0.01" {...register("compareAtPrice")} className="w-full px-3 py-2 border border-cream-200 rounded-lg text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Quantity *</label>
              <input type="number" {...register("quantity")} className="w-full px-3 py-2 border border-cream-200 rounded-lg text-sm" />
              {errors.quantity && <p className="text-xs text-red-600 mt-1">{errors.quantity.message}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">SKU</label>
              <input {...register("sku")} className="w-full px-3 py-2 border border-cream-200 rounded-lg text-sm" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-cream-200 p-6 space-y-4">
          ...
          <div>
            ...
            <input {...register("metaTitle")} maxLength={70} className="w-full px-3 py-2 border border-cream-200 rounded-lg text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Meta Description</label>
            <textarea {...register("metaDescription")} maxLength={160} rows={2} className="w-full px-3 py-2 border border-cream-200 rounded-lg text-sm" />
          </div>
        </div>
      </form>
    </motion.div>
