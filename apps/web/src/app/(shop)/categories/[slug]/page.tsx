    ...
    <>
      <div className="relative overflow-hidden">
        {category.image ? (
          <div className="relative h-48 md:h-64">
            ...
            <div className="absolute inset-0 bg-gradient-to-r from-text-900/70 to-transparent" />
          </div>
        ) : (
          <div className="h-48 md:h-64 bg-gradient-to-r from-primary-600 to-accent-500" />
        )}
        <div className="absolute top-0 left-0 right-0 p-6 md:p-8">
          <div className="mx-auto max-w-7xl">
            <Breadcrumb items={[{ label: "Home", href: "/" }, { label: "Categories", href: "/categories" }, { label: category.name }]} className="text-surface-200" />
            <h1 className="mt-3 font-serif text-3xl font-bold text-white md:text-4xl">{category.name}</h1>
            {category.description && <p className="mt-2 max-w-xl text-sm text-surface-200">{category.description}</p>}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white py-12">
        <div className="mx-auto max-w-7xl px-4">
          {category.children && category.children.length > 0 && (
            <div className="mb-8 flex flex-wrap gap-2">
              {category.children.map((sub) => (
                <Link key={sub.id} href={`/categories/${sub.slug}`} className="rounded-full border border-text-200 px-4 py-1.5 text-sm text-text-600 transition-colors hover:border-primary-400 hover:bg-primary-50 hover:text-primary-700">
                  {sub.name}
                </Link>
              ))}
            </div>
          ...
          <h2 className="font-serif text-2xl font-bold text-text-900">Top Picks in {category.name}</h2>
          <div className="mt-6">
            {products.length > 0 ? (
