  ...
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-cream-50">
        <div className="h-10 w-10 animate-spin rounded-full border-4 border-charcoal-200 border-t-rose-600" />
      </div>
    );
  }
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-cream-50 px-4">
        <div className="text-center">
          ...
          <Link href="/" className="mt-4 inline-block text-sm font-medium text-rose-600 hover:underline">Go Home</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-cream-50">
      <div className="mx-auto max-w-3xl px-4 py-12">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mt-8 rounded-2xl bg-white p-6 shadow-soft-sm">
          <div className="flex items-center justify-between border-b border-charcoal-100 pb-4">
            <div>
              ...
              <button onClick={handleCopy} className="flex items-center gap-2 font-mono text-lg font-bold text-charcoal-900 hover:text-rose-600">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <div className="flex items-start gap-3">
              ...
              <div><p className="text-xs text-muted-500">Order Date</p><p className="text-sm font-medium text-charcoal-800">{formatDate(order.createdAt)}</p></div>
            </div>
            <div className="flex items-start gap-3">
              <Truck className="mt-0.5 h-4 w-4 text-muted-400" />
              <div><p className="text-xs text-muted-500">Est. Delivery</p><p className="text-sm font-medium text-charcoal-800">{deliveryEstimate}</p></div>
            </div>
            <div className="flex items-start gap-3">
              <MapPin className="mt-0.5 h-4 w-4 text-muted-400" />
              <div><p className="text-xs text-muted-500">Shipping To</p><p className="text-sm font-medium text-charcoal-800">{order.shippingAddress?.city}, {order.shippingAddress?.district}</p></div>
            </div>
            <div className="flex items-start gap-3">
              <CreditCard className="mt-0.5 h-4 w-4 text-muted-400" />
              <div><p className="text-xs text-muted-500">Payment</p><p className="text-sm font-medium text-charcoal-800">{order.paymentMethod || "Online Payment"}</p></div>
            </div>
          </div>
        </motion.div>
            ...
            <div className="mt-4 space-y-3">
              {order.items.map((item) => (
                <div key={item.id} className="flex items-center gap-4 border-b border-charcoal-100 pb-3 last:border-0 last:pb-0">
                  ...
                  <div className="flex-1"><p className="text-sm font-medium text-charcoal-800">{item.productTitle}</p><p className="text-xs text-muted-500">Qty: {item.quantity}</p></div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center">
          <Link href={`/dashboard/orders/${order.id}`} className="inline-flex items-center justify-center gap-2 rounded-lg bg-rose-600 px-6 py-3 text-sm font-semibold text-white shadow-soft hover:bg-rose-700">
          ...
          <div className="mt-6 flex justify-between">
            {timelineSteps.map((step, i) => (
              <div key={step.label} className="flex-1 text-center">
                <div className={`mx-auto flex h-10 w-10 items-center justify-center rounded-full ${step.done ? "bg-muted-100 text-muted-600" : "bg-charcoal-100 text-muted-400"}`}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="mt-10 rounded-2xl bg-blush-50 p-6 text-center">
          <h2 className="font-serif text-lg font-bold text-blush-800">Share the Love</h2>
          <p className="mt-1 text-sm text-blush-600">Tell your friends about Loopingon and earn rewards!</p>
