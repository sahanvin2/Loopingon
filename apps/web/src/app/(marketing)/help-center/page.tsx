    ...
    <>
      <section className="bg-white py-16">
        <div className="mx-auto max-w-4xl px-4 text-center">
          ...
          <p className="mt-4 text-lg text-warm-gray-600">How can we help you today?</p>
          <div className="relative mx-auto mt-8 max-w-xl">
            <Search className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-warm-gray-400" />
            <input
              type="text"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search for answers..."
              className="w-full rounded-xl border border-charcoal-200 bg-cream-100 py-3.5 pl-12 pr-4 text-sm text-charcoal-900 placeholder:text-warm-gray-400 focus:border-terracotta-500 focus:ring-2 focus:ring-terracotta-500/20 focus:outline-none"
            />
          </div>
        </div>
      </section>
      ...
      <section className="bg-cream-100 py-16">
        <div className="mx-auto max-w-3xl px-4">
          {filteredSections.length === 0 && (
            <p className="py-12 text-center text-warm-gray-500">No results found. Try different keywords or browse the categories below.</p>
          )}
              ...
              <div key={section.title} className="overflow-hidden rounded-xl bg-white shadow-soft-sm">
                ...
                >
                  <span className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-warm-gray-400">({section.questions.length})</span>
                  </span>
                  <motion.div
                      ...
                      <div className="space-y-1 px-6 pb-4">
                        {section.questions.map((faq) => (
                          <details key={faq.q} className="group border-t border-charcoal-100 pt-3 first:border-t-0 first:pt-0">
                            <summary className="cursor-pointer py-2 text-sm font-medium text-charcoal-700 hover:text-terracotta-600">
                              {faq.q}
                            </summary>
                            <p className="pb-2 text-sm leading-relaxed text-warm-gray-600">{faq.a}</p>
                          </details>
                        ))}
                      </div>
                    </motion.div>
      ...
      <section className="bg-white py-16">
        <div className="mx-auto max-w-2xl px-4 text-center">
          <div className="rounded-2xl bg-cream-100 p-8 md:p-10">
            <HeadphonesIcon className="mx-auto h-10 w-10 text-terracotta-600" />
            <h2 className="mt-4 font-serif text-2xl font-bold text-charcoal-900">Still Need Help?</h2>
            <p className="mt-2 text-warm-gray-600">Our support team is here to help you with any questions or issues.</p>
            <div className="mt-6 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
              <Link
                href="/contact"
                className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-terracotta-600 px-6 py-3 text-sm font-semibold text-white shadow-soft transition-all hover:bg-terracotta-700 sm:w-auto"
              >
