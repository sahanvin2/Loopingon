...
function CompetitionCard({ competition }: { competition: Competition }) {
  return (
    <Link href={`/competitions/${competition.slug}`} className="group overflow-hidden rounded-xl bg-cream-50 shadow-soft-sm transition-shadow hover:shadow-soft">
      ...
      <div className="p-5">
        {competition.craftType && (
          <span className="mb-2 inline-block rounded-full bg-rose-50 px-2.5 py-0.5 text-xs font-medium text-rose-600">{competition.craftType}</span>
        )}
        <h3 className="font-serif text-lg font-bold text-charcoal-900 group-hover:text-rose-600 transition-colors">{competition.title}</h3>
        ...
        <div className="mt-4 border-t border-charcoal-100 pt-3">
          <div className="flex justify-between text-xs text-muted-500">
            ...
            {competition.prizeDescription && <span className="font-semibold text-blush-600">Prizes Available</span>}
          </div>
        </div>
      </div>
    </Link>
    ...
    <>
      ...
      <section className="bg-cream-50 py-16">
        <div className="mx-auto max-w-2xl px-4 text-center">
          ...
          <Link href="/sign-up/vendor" className="mt-6 inline-flex items-center rounded-lg bg-rose-600 px-8 py-3 text-sm font-semibold text-white shadow-soft transition-all hover:bg-rose-700">
            Become a Vendor
          </Link>
        </div>
      </section>
