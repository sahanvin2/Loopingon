      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          >
            <button
              onClick={closeModal}
              className="absolute right-4 top-4 grid h-9 w-9 place-items-center rounded-full bg-blush-50 hover:bg-blush-100 transition-colors z-10"
              aria-label="Close"
            >
            ...
            <div className="mb-5 text-center">
              <span className="inline-flex items-center justify-center h-10 w-10 rounded-full bg-gradient-to-br from-rose-400 to-rose-600 text-white font-serif text-lg shadow-[0_8px_20px_-8px_rgba(176,86,110,0.6)] mb-3">L</span>
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          <label htmlFor="modal-email" className="block text-sm font-medium text-charcoal-700 mb-1.5">Email address</label>
          <div className="relative">
            ...
            <input
              ...
              className={cn("w-full rounded-xl border py-3 pl-10 pr-4 text-sm text-charcoal-900 placeholder:text-muted-400 focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 focus:outline-none transition-shadow", errors.email ? "border-red-400" : "border-blush-200")}
            />
          </div>
        ...
        <div>
          <label htmlFor="modal-password" className="block text-sm font-medium text-charcoal-700 mb-1.5">Password</label>
          <div className="relative">
            ...
            <input
              ...
              className={cn("w-full rounded-xl border py-3 pl-10 pr-12 text-sm text-charcoal-900 placeholder:text-muted-400 focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 focus:outline-none transition-shadow", errors.password ? "border-red-400" : "border-blush-200")}
            />
            <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-muted-400 hover:text-charcoal-600">
        ...
        <button
          ...
          className="flex h-12 w-full items-center justify-center rounded-xl bg-rose-500 text-sm font-semibold text-white shadow-sm shadow-rose-500/25 transition-all hover:bg-rose-600 hover:shadow-md hover:shadow-rose-500/30 disabled:cursor-not-allowed disabled:opacity-60"
        >
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <button className="flex h-11 items-center justify-center gap-2 rounded-xl border border-blush-200 bg-white text-sm font-medium text-charcoal-700 transition-colors hover:bg-blush-50">
      ...
      <p className="mt-6 text-center text-sm text-muted-500">
        ...
        <button onClick={onSwitch} className="font-semibold text-rose-600 hover:text-rose-700">Create one</button>
      </p>
    </div>
  );
}
...
function SignUpForm({ onSwitch, onSuccess }: { onSwitch: () => void; onSuccess: () => void }) {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          <label htmlFor="modal-signup-name" className="block text-sm font-medium text-charcoal-700 mb-1.5">Full Name</label>
          <div className="relative">
            ...
            <input id="modal-signup-name" autoComplete="name" {...register("fullName")} placeholder="John Doe"
              className={cn("w-full rounded-xl border py-3 pl-10 pr-4 text-sm text-charcoal-900 placeholder:text-muted-400 focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 focus:outline-none transition-shadow", errors.fullName ? "border-red-400" : "border-blush-200")} />
          </div>
          {errors.fullName && <p className="mt-1 text-xs text-red-500">{errors.fullName.message}</p>}
        </div>

        <div>
          <label htmlFor="modal-signup-email" className="block text-sm font-medium text-charcoal-700 mb-1.5">Email address</label>
          <div className="relative">
            <Mail className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-400" />
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label htmlFor="modal-signup-cpw" className="block text-sm font-medium text-charcoal-700 mb-1.5">Confirm</label>
            <div className="relative">
              <Lock className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-400" />
              <input id="modal-signup-cpw" type={showConfirmPassword ? "text" : "password"} autoComplete="new-password" {...register("confirmPassword")} placeholder="Re-enter"
                className={cn("w-full rounded-xl border py-3 pl-10 pr-10 text-sm text-charcoal-900 placeholder:text-muted-400 focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 focus:outline-none transition-shadow", errors.confirmPassword ? "border-red-400" : "border-blush-200")} />
        ...
        {pwStrength && (
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-xs font-medium text-charcoal-700">{pwStrength.label}</span>
            </div>
        ...
        <label className="flex items-start gap-2.5 cursor-pointer">
          ...
          <span className="text-xs text-muted-600 leading-relaxed">
            I agree to the{" "}
            <a href="/terms-of-service" target="_blank" className="text-rose-600 hover:underline font-medium">Terms of Service</a>
            {" "}and{" "}
            <a href="/privacy-policy" target="_blank" className="text-rose-600 hover:underline font-medium">Privacy Policy</a>
          </span>
        </label>
      ...
      <p className="mt-6 text-center text-sm text-muted-500">
        ...
        <button onClick={onSwitch} className="font-semibold text-rose-600 hover:text-rose-700">Sign in</button>
      </p>

      <p className="mt-3 text-center text-xs text-muted-400">
        Want to sell your crafts?{" "}
        <a href="/sign-up/vendor" target="_blank" className="font-medium text-rose-600 hover:text-rose-700">Apply as a vendor →</a>
      </p>
    </div>
  );
}
