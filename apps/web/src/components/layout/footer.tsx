...
export function Footer() {
  ...
  return (
    <footer className="mt-24 border-t border-accent-200 bg-accent-100/40">
      <div className="max-w-8xl mx-auto px-4">
        <div className="grid gap-10 py-14 md:grid-cols-3 lg:grid-cols-5">
          {/* Brand */}
          <div>
            <div className="flex items-center gap-2">
              <span className="grid h-9 w-9 place-items-center rounded-full bg-gradient-to-br from-primary-400 to-primary-600 text-white font-serif text-lg shadow-[0_8px_20px_-8px_rgba(176,86,110,0.7)]">
                L
              </span>
              <span className="font-serif text-2xl tracking-tight text-text-900">
                loopingo
              </span>
            </div>
          ...
          {footerColumns.map((col) => (
            <div key={col.title}>
              <h4 className="font-serif text-lg text-text-800">{col.title}</h4>
        ...
        {/* Bottom bar */}
        <div className="border-t border-accent-200 py-5 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            {socialLinks.map((social) => (
              <a
                ...
                className={cn(
                  "grid h-10 w-10 place-items-center rounded-full border border-accent-200 bg-white text-muted-400 hover:text-primary-500 hover:border-primary-200 transition-all shadow-sm",
                )}
          ...
          <div className="flex items-center gap-3">
            {["Visa / MC", "Amex", "PayHere"].map((method) => (
              <span
                key={method}
                className="h-8 px-3 rounded-full bg-white border border-accent-200 flex items-center text-muted-500 text-xs font-medium tracking-wider shadow-sm"
              >
                {method}
              </span>
            ))}
