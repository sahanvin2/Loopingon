...
async function run() {
  try {
    ...
    console.log('Testing network routing inside docker container...');
    const res = await ssh.execCommand('docker exec loopingon-server-prod ping -c 1 db.lbrggticuwyqmdtllxsh.supabase.co');
    ...
    const res2 = await ssh.execCommand('docker exec loopingon-server-prod ping -c 1 aws-0-ap-southeast-1.pooler.supabase.com');
